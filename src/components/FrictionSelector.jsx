...
const FrictionSelector = ({ stuckInput, onContinue, isLoading }) => {
  const [selected, setSelected] = useState('');
  const [customInput, setCustomInput] = useState('');

  const showCustomInput = selected === 'Something else';

  const handleContinue = () => {
    const frictionInput = showCustomInput ? customInput : selected;
    if (frictionInput.trim()) {
      onContinue(frictionInput);
    }
  };

  const isDisabled = !selected || (showCustomInput && !customInput.trim());
...
export default FrictionSelector;
